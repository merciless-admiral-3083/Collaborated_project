...
from database import db  # adjust based on your actual DB import
...
def get_retrain_status(current_user: dict = Depends(require_role("admin"))):

    latest_log = db.retrain_logs.find_one(
        {},
        sort=[("started_at", -1)]
    )

    if not latest_log:
        return {"message": "No retraining history"}
