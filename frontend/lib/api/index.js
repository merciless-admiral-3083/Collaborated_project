...
// Helper to parse JSON responses and handle errors
async function parseResponse(res) {
  const text = await res.text();
  try {
    const json = JSON.parse(text);
    ...
    // Not valid JSON — show helpful message
    const err = new Error(
      `Non-JSON response (status ${res.status}). Body starts with: ${text.slice(
        0,
        200
      )}`
    );
...
export async function apiPost(path, body = {}, opts = {}) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify(body),
    // Remove credentials unless you specifically need cookies
    // credentials: opts.credentials || "include",
  });
  return parseResponse(res);
}
